...
/** Movie + cleanup must finish before the next slot (or midnight for last slot). */
...
export function generateShowtimes(input: ScheduleInput): GeneratedShowtime[] {
  ...
  if (validSlots.length === 0) {
    validSlots = [...SHOWTIME_SLOTS];
  }

  let screens = input.screens;
  if (input.cinemaId) {
    screens = screens.filter((s) => s.cinemaId === input.cinemaId);
  }
  ...
  for (let dayOffset = 0; dayOffset < 14 && picked.length < input.count; dayOffset++) {
    ...
    const slotsBefore = picked.length;
    ...
    // Prefer filling one day with distinct slots before moving to the next day
    if (!input.preferDate && picked.length >= input.count) break;
    if (!input.preferDate && picked.length > slotsBefore && picked.length < input.count) {
      // partial day — continue next day for remaining slots
    }
  }
...
/** Each showtime should use a different clock time (HH:mm). */
